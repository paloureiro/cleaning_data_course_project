...
```
The original table does not contain column headers that identify the measurements. Those are read from another file (features.txt) that is "cleaned up" by a series of character substitutions. The file CodeBook.md gives a succint description of the names of the headers. 
```R
